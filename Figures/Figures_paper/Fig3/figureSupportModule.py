...
def makeLayout5(labelsOptions=dict(), **figkwargs):
    ...
    legendGrid = mainGrid[0, :].subgridspec(1, 2, wspace=0.01)
...
def makeLayout2_test(labelsOptions=dict(), **figkwargs):
    # axes = dict()
    fig = plt.figure(**figkwargs)
    # npGrid = mainGrid[0, 0].subgridspec(1, 3)
    # tmatGrid = mainGrid[2, 0].subgridspec(1, 3, width_ratios=[1, 1, 0.05])
...
def makeLayout6and7(labelsOptions=dict(), temps=[300, 500], **figkwargs):
    axes = dict()
    fig = plt.figure(**figkwargs)
    mainGrid = fig.add_gridspec(
        nrows=3,
        ncols=2,
        height_ratios=[0.9, 1.7, 1.7],
        width_ratios=[2.9, 1.0],
        wspace=0.18,
    )
    npGrid = mainGrid[0, 0].subgridspec(1, 3)
    tmatGrid = mainGrid[2, 0].subgridspec(1, 3, width_ratios=[1, 1, 0.03])
    chordGrid = mainGrid[:, 1].subgridspec(2, 1)
    axes[f"npIdeal"] = fig.add_subplot(npGrid[0])
...
def makeLayout7(labelsOptions=dict(), temps=[300, 400, 500], **figkwargs):
    axes = dict()
    fig = plt.figure(**figkwargs)
    mainGrid = fig.add_gridspec(
        nrows=4,
        ncols=1,
        height_ratios=[0.5, 1.5, 1.1, 1],
        width_ratios=[1],
        wspace=0.1,
    )
    npGrid = mainGrid[0, 0].subgridspec(1, 3)
    tmatGrid = mainGrid[2, 0].subgridspec(1, 3, width_ratios=[1, 1, 0.02])
    chordGrid = mainGrid[3, 0].subgridspec(1, 2)
    axes[f"npIdeal"] = fig.add_subplot(npGrid[0])
    for i, T in enumerate(temps):
        axes[f"np{T}"] = fig.add_subplot(npGrid[i + 1])
        axes[f"tmat{T}"] = fig.add_subplot(tmatGrid[i])
...
def makeLayoutSI_onlyT(labelsOptions=dict(), temps=[300], **figkwargs):
    axes = dict()
    fig = plt.figure(**figkwargs)
    mainGrid = fig.add_gridspec(
        nrows=3,
    ...
    npGrid = mainGrid[0, :].subgridspec(1, 2)
    tmatGrid = mainGrid[2, 0].subgridspec(1,1)#, width_ratios=[1,0.04])
    chordGrid = mainGrid[2, 1].subgridspec(1, 1)
    axes[f"npIdeal"] = fig.add_subplot(npGrid[0])
    for i, T in enumerate(temps):
        axes[f"np{T}"] = fig.add_subplot(npGrid[i + 1])
        axes[f"tmat{T}"] = fig.add_subplot(tmatGrid[i])
        axes[f"chord{T}"] = fig.add_subplot(chordGrid[i])
    #axes[f"tmatCMAP"] = fig.add_subplot(tmatGrid[1])
    axes[f"nps"] = fig.add_subplot(npGrid[:])
    axes[f"tmats"] = fig.add_subplot(tmatGrid[:])
    axes[f"chords"] = fig.add_subplot(chordGrid[:])
    axes["Histo"] = fig.add_subplot(mainGrid[1, :])
    titleDict = __titleDict.copy()
    titleDict["fontdict"].update(labelsOptions)
    for i, ax in enumerate(
        [
...
def makeLayout3_onlyT(labelsOptions=dict(), temps=[300], **figkwargs):
    axes = dict()
    fig = plt.figure(**figkwargs)
    mainGrid = fig.add_gridspec(
        nrows=3,
    ...
    npGrid = mainGrid[0, :].subgridspec(1, 2)
    tmatGrid = mainGrid[1].subgridspec(1, 2, width_ratios=[1,0.04])
    chordGrid = mainGrid[2].subgridspec(1, 1)
    axes[f"npIdeal"] = fig.add_subplot(npGrid[0])
    for i, T in enumerate(temps):
        axes[f"np{T}"] = fig.add_subplot(npGrid[i + 1])
        axes[f"tmat{T}"] = fig.add_subplot(tmatGrid[i])
...
def HistoMaker(
    ...
    ax.set_xticks(
        range(nHisto),
        [topDownLabels[positions.index(k)] for k in range(nHisto)],
        fontsize=25,
    )
    styles = {
        "Ideal": dict(edgecolor="k", alpha=0.8),
