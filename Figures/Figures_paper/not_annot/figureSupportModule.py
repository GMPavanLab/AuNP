...
topDownColorMap_nobulk = numpy.array([to_rgb(k) for k in topDownColorMapHex_nobulk])
...
topDownReordering_nobulk = [1, 2, 5, 3, 4, 6, 7, 9, 8]
# [0, 1, 2, 4, 5, 3, 6, 7, 9, 8]
...
def makeLayout6and7(labelsOptions=dict(), temps=[300, 400, 500], **figkwargs):
    ...
    mainGrid = fig.add_gridspec(
        nrows=3,
        ncols=2,
        height_ratios=[0.6, 1.9, 1.6],
        width_ratios=[2.0, 1.0],
        wspace=0.3,
    )
    npGrid = mainGrid[0, 0].subgridspec(1, 3)
    tmatGrid = mainGrid[2, 0].subgridspec(1, 2, width_ratios=[1, 1])
    chordGrid = mainGrid[:, 1].subgridspec(2, 1)
    axes[f"npIdeal"] = fig.add_subplot(npGrid[0])
...
def makeLayout7(labelsOptions=dict(), temps=[300, 400, 500], **figkwargs):
    ...
    mainGrid = fig.add_gridspec(
        nrows=4,
        ncols=1,
        height_ratios=[0.5, 1.2, 1.2, 1.1],
        width_ratios=[0.9],
        wspace=0.2,
    )
    npGrid = mainGrid[0, 0].subgridspec(1, 3)
    tmatGrid = mainGrid[2, 0].subgridspec(1, 3, width_ratios=[0.9, 0.9, 0.04])
...
def AddTmatsAndChord5_6_7(
    ...
    tmatOpts = dict(
        ...
        cbar=cbarAx is not None,
        cbar_ax=cbarAx,
        xticklabels=topDownLabels if T != 673 else False,
        yticklabels=topDownLabels if T != 673 else False,
        annot_kws={"size": 16},
    )
    tmatOpts.update(tmatOptions)
    seaborn.heatmap(
...
def HistoMaker(
    ...
    ax.set_xlim(right=nHisto - 1 + barsDim * scaleAx, xmin=0.55)
    # ax.set_yticks([])
...
def HistoMaker_only(
    ...
    scaleAx = 0.58
    # ax.set_xlim(-barsDim * scaleAx, nHisto - 1 + barsDim * scaleAx)
    ax.set_xlim(-barsDim * scaleAx, xmax=9.5)
    # ax.set_yticks([])
    ax.set_ylabel("Mean Number of Atoms")
    for c in range(nHisto):
        if countMean["Ideal"][c] == 0:
