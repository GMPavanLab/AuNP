...
def HistoMaker673(
    ...
    #ax.set_xticks(
    #    range(nHisto),
    #    [topDownLabels[p] for p in positions],
    #    fontsize=25,
    #)
    styles = {
        "Ideal": dict(edgecolor="k", alpha=0.8),
    ...
    scaleAx = 0.6
    ax.set_xticks(
    #ax.set_xticks(range(nHisto))
    #ax.set_xticklabels([topDownLabels[p] for p in positions], fontsize=25)
        range(nHisto),
        [topDownLabels[p] for p in positions],
        fontsize=25,
    )
...
def HistoMakermean300(
    ax: plt.Axes,
    data: numpy.ndarray,  # Pass the data to be plotted as a NumPy array
    positions=None,
    barWidth: float = 0.15,
    barSpace: float = 0.05,
):
    ...
    for T in order:
        ax.bar(
            ...
            width=barWidth,
            align="center",
            color=topDownColorMap,  # Replace with your desired colors
            **styles[T],
        )
